...
class Populate:
    ...
    def createNew(dt=.1):
        ...
        return Populate(V((random()-.5) * 8, (random()-.5)*8, 0), dt=dt, mass=mass)
    ...
    def collision(self, walls=None):
        ...
        # Left wall of bunker
        if ((self.position + self.velocity * self.dt).x > 1 and (self.position.x < 1)) and self.position.y < 4 and self.position.y > 2:
        ...
        # Right wall of bunker
        if ((self.position + self.velocity * self.dt).x > 2 and (self.position.x < 2)) and self.position.y < 4 and self.position.y > 2:
            # collision from the left
            Nn = V(-1, 0, 0)
            rNn = Nn.rotate((0.5-random())*WALL_RANDOM_FACTOR*2)
            speed_kept = 1 - .2 * abs(self.velocity() * rNn)
        ...
        # Bottom wall of bunker
        if ((self.position + self.velocity * self.dt).y > 2 and (self.position.y < 2)) and self.position.x > 1 and self.position.y < 2:
            # collision from bottom
            Nn = V(0, -1, 0)
            rNn = Nn.rotate((0.5-random())*WALL_RANDOM_FACTOR*2)
            speed_kept = 1 - .2 * abs(self.velocity() * rNn)
            self.velocity += 2 * rNn * abs(self.velocity * rNn) * speed_kept
        if ((self.position + self.velocity * self.dt).y < 2 and (self.position.y > 2)) and self.position.x > 1 and self.position.x < 2:
            # collision from top
            Nn = V(0, 1, 0)
            rNn = Nn.rotate((0.5-random())*WALL_RANDOM_FACTOR*2)
            speed_kept = 1 - .2 * abs(self.velocity() * rNn)
    ...
    def update(self):
        ...
        if self.position.x > 1 and self.position.x < 2 and self.position.y < 4 and self.position.y > 2:
            self.setColor("blue")
    ...
    def getScore(self):
        if not (self.position.x > 1 and self.position.x < 2 and self.position.y < 4 and self.position.y > 2):
            if self.position.y < 4:
                return self.position - holePosition + V(1000, 0, 0)
            else:
